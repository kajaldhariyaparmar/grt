...
class PaymentController extends Controller
{
    ...
    public function singlePaymentProcess(Request $request)
    {
        ...
        if($request->payment_type == 'PayPal'){
            $paymentdata = $request->id;
           //for Paypal 
           $payment_method = 'PayPal'; 
           // CRM webhook
           $result = $this->crmwebhook_single($request->reference, $request->user, $request->cartContent, $request->currency, $paymentdata,$request->payment_type);
            ...
            $paymentdata = $s->call();
            $payment_method = $paymentdata['payment_method_details']['card']['brand'];
        ...
        $this->donationsInsertDatabase($request->reference, $request->user, $request->cartContent, $request->cartTotal, $request->currency,$payment_method);
    ...
    public function monthlyPaymentProcess(Request $request)
    {
        ...
        if(isset($gocardless_result->mandate)) {
            ...
            $this->donationsInsertDatabase($request->reference, $request->user, $request->cartContent, $request->cartTotal, $request->currency,'Gocardless');
     ...
     * @return 
     */
    public function crmwebhook_single($reference, $userData, $cartContent, $currency, $paymentdata,$payment_type) {
        // Call webhook
        ...
        foreach($cartContent as $row) {
            if($row['paymenttype'] == "One-off") {
                ...
                if(isset($row['countries'])) {
                    ...
                    $items[] = array('amount' => $row['totalPrice'],'item' => $row['title'], 'type' => 'fund', "FundDimensions" => $funddimensions);
        ...
        if($payment_type == 'PayPal'){
            ...
            $crmwebhook->data['payment'] = array(
                ...
                'stripe' => array(
                    ...
                    "PaymentMethodId" => $paymentdata['id']//$paymentdata['payment_method'],
                    // "PaymentIntentId" => "",
                    )
            );
        }
     ...
     */
    public function donationsInsertDatabase($reference, $userData, $cartContent, $cartTotal, $currency,$payment_method)
    {
        $now = new DateTime();
        ...
        foreach($cartContent as $row) {
            ...
            $donation_data[$i]['payment_method'] = $payment_method;
            ...
            $donation_data[$i]['title_name'] = $userData['customertitle'];
        ...
        $data = Donation::insert($donation_data);
        ini_set('display_errors', 1);
        ini_set('display_startup_errors', 1);
        error_reporting(E_ALL);
        //Now Upload receipt
        //ReceiptController::uploadReceipt($i);
        $sender_name = $userData['name'];
        $sender_email = $userData['email'];
        

        // sending email to admin 
        $path = storage_path('app/'.'receipt-admin.pdf');
        Mail::send('emails.admin', array('reference' => $reference, 'admincost' => $adminCost, 'order' => $userData , 'cart_data' => $cartContent), function ($message) use ($sender_name, $sender_email,$path) {
            // $message->from($sender_email, $sender_name);
            $message->from('info@grtuk.org', 'Global Relief Trust');
            $message->subject('Got a new donation');
            $message->to('kajal.dhariya@togglebytes.com');
            //$message->attach($path);
        }); 

        // sending email to donor 
        $path = storage_path('app/'.'receipt-user.pdf');
        Mail::send('emails.thankyou', array('admincost' => $adminCost, 'order' => $userData , 'cart_data' => $cartContent), function ($message) use ($sender_name, $sender_email,$path) {
            $message->from('info@grtuk.org', 'Global Relief Trust');
        ...
        // Now Code for generate or upload receipt pdf 

    }
    public function uploadReceipt($count)
    {
        // Setup a filename 
        ...
        $document->WriteHTML($response);
         
        // Save PDF on your public storage 
        Storage::disk('local')->put($documentFileName, $document->Output($documentFileName, "S"));
        // // Get file back from storage with the give header informations
        // return Storage::disk('local')->download($documentFileName, 'Request', $header);
    ...
    public function fetchDetails(Request $req)
    {
        ...
        \Stripe\Stripe::setApiKey('sk_test_51MRV9gSGmv8zsy3K0o5xGvYWfyfQoXdawf5nfi20fyMRXSGQvZEHyzobhunP6GwNzrBp2XJH8m0aNrJa53sJUzgX00D5o3VxSr');
...
class Gocardless_webhook {
    ...
    //public $access_token = "pk_test_51MRV9gSGmv8zsy3KCjjc7pAg896OiVC5iXlALKXdxmVHqaE2vq2CWOwzE73PkNHAT1W4Voqnayau0pYhuFyVCzwQ00VOQm9X4Q";
