<template>
  <div>
    ...
    <section
      v-if="userName"
      class="cf-section-wrap cf-dashboard-header-section"
    >
      <div class="container">
        <div class="row">
          <div class="col-sm-12">
                  ...
                  <ul>
                    <span v-for="value in cfmembers" v-bind:key="value.id">
                      ...
                      >
                        <li
                          v-if="
                            value.is_team == 'yes' && value.member_id == userId
                          "
                        >
                          <div class="cf-dashboard-item-wrap">
                            <div class="cf-thumb-col cf-column">
...
export default {
  ...
  created() {
    this.axios
      .get(this.base_url +`/fundraising/author/${this.$route.params.id}`)
      .then((response) => {
        this.loading = false;
  ...
  methods: {
    ...
    totalTeamCampaigns(cfmembers) {
      ...
      for (let value of cfmembers) {
        if (value.cfcampaigns.length > 0) {
          for (let value of value.cfcampaigns) {
            if (value.is_team == "yes" && value.member_id == this.userId) {
              totalCampaigns += 1;
            }
          }
        }
